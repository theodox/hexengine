...
```text
MoveUnit / movement arc  →  MovementHook.*  →  hooks/movement.py  →  ../movement_rules.py (+ ../session_state.py retreat reads)
...
1. **Shared policy** — add or extend a pack-root module (`session_state.py`, …): pure `GameState` in/out.
...
- Policy modules: `../marker_rules.py`, `../session_state.py`
