...
``hooks/movement.py`` binds these functions to ``MovementHook`` slots. Preview RPCs stay
...
class HexdemoMovementRules:
    """Concrete binding for docs and tests (hook adapters call module functions)."""

    movement_step_cost_for_unit = staticmethod(movement_step_cost_for_unit)
    movement_budget_for_unit = staticmethod(movement_budget_for_unit)
    zoc_hexes_for_unit = staticmethod(zoc_hexes_for_unit)
    retreat_obligation_hexes_remaining = staticmethod(
        retreat_obligation_hexes_remaining
    )
    any_retreat_obligation_pending = staticmethod(any_retreat_obligation_pending)
    faction_has_pending_retreat_obligation = staticmethod(
        faction_has_pending_retreat_obligation
    )
    retreat_blocked_hexes = staticmethod(retreat_blocked_hexes)
    validate_retreat_move = staticmethod(validate_retreat_move)
    auto_advance_phase_after_move_spend = staticmethod(
