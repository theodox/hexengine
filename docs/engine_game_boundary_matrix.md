...
| 4 | Turn schedule (game ↔ engine handshake) | — | [`GameDefinition.get_next_phase`](src/hexengine/gamedef/protocol.py) implemented in [`src/hexengine/gamedef/builtin.py`](src/hexengine/gamedef/builtin.py); [`advance_turn_action_for_state`](src/hexengine/gamedef/builtin.py) for network UI | Server: [`_get_next_phase`](src/hexengine/server/game_server.py) → `_game_definition.get_next_phase`; after `MoveUnit` + `SpendAction`, auto [`NextPhase`](src/hexengine/server/game_server.py) when actions depleted. **Local** client: [`game.Game.advance_turn`](src/hexengine/game/game.py) indexes `TurnManager.phases`. **Network** client: [`NetworkGame.advance_turn`](src/hexengine/game/network_game.py) mirrors the server from [`StateUpdate.turn_rules`](src/hexengine/server/protocol.py) (schedule + factions + movement budget); switching titles without reload is out of scope | med | Align local `Game.advance_turn` with title-owned phases when titles diverge from engine `TurnManager` |
