...
| 6 | Movement legality | Terrain on board / `LocationState` from scenario | `ModificationHook` (`movement_budget_for_unit`, ZOC, step cost); hexdemo binds in [`hooks/modification.py`](games/hexdemo/hooks/modification.py) (unit `movement` attribute) | Server [`_validate_move_unit_request`](src/hexengine/server/game_server.py) before `MoveUnit`. Client preview via **`unit_preview_request`** → [`server/preview.py`](src/hexengine/server/preview.py) (no local `compute_valid_moves`) | med | Stepwise arc + stricter catalog defaults; keep preview/validation in one module (done for preview) |
