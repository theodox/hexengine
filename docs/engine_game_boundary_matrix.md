...
Per-domain authority inventory (engine vs title vs pack). Rows were checked against the codebase (paths relative to repo root).
...
| 4 | Turn schedule (game ↔ engine handshake) | — | [`arcs/turn_schedule.py`](games/hexdemo/arcs/turn_schedule.py) declares rota; [`ArcHook.TURN_ARC_REGISTRY`](games/hexdemo/arcs/wiring.py) + [`segment_blocks_routine_phase_advance`](src/hexengine/arcs/segment_wire.py) | Server: [`NextPhase`](src/hexengine/server/game_server.py) rejected when active segment omits `NextPhase`; auto-advance after move/attack via modification/interaction hooks using [`arcs/segment.phase_advance_blocked`](games/hexdemo/arcs/segment.py) | med | — |
| 5 | Action economy (handshake) | — | Title owns auto-advance policy via modification/interaction hooks; blocked when [`arcs/segment.phase_advance_blocked`](games/hexdemo/arcs/segment.py) (active segment omits `NextPhase`) | [`GameServer._handle_action_request`](src/hexengine/server/game_server.py): after `execute(MoveUnit)`, `execute(SpendAction(1))`; then [`_maybe_auto_advance_phase`](src/hexengine/server/game_server.py) | med | Configurable spend amounts per action type |
| 6 | Movement legality | Terrain on board / `LocationState` from scenario | `ModificationHook` (`movement_budget_for_unit`, ZOC, step cost); hexdemo binds in [`hooks/modification.py`](games/hexdemo/hooks/modification.py) (unit `movement` attribute) | Server [`_validate_move_unit_request`](src/hexengine/server/game_server.py) before `MoveUnit`; stepwise open/continue via movement arc (`SEG_STEPWISE_OPEN`, `SEG_RETREAT_OPEN`, `drive_movement_arc_*`). Client preview via **`unit_preview_request`** → [`server/preview.py`](src/hexengine/server/preview.py) | med | Configurable spend amounts; stricter catalog defaults for non-opt-in titles |
...
| 9 | Affordances (`InteractionKind`+) | — | Preview policy per `InteractionKind` via bound hooks; hexdemo implements attack plan, retreat path, place marker; combat gate dock rows via [`combat_gate_panel_actions`](src/hexengine/authoring/patterns/combat.py) in title dock hook | [`InteractionKind`](src/hexengine/gamedef/interactions.py) enum; server dispatch [`map_selection_registry.py`](src/hexengine/hooks/map_selection_registry.py) → `map_selection_preview_request`; client apply table in [`client_map_selection_registry.py`](src/hexengine/game/arcs/client_map_selection_registry.py). **Draft storage = client; preview = consult; commit = authority** ([`TURN_ACTION_DOCK_CONTRACT.md` § Draft locus](docs/TURN_ACTION_DOCK_CONTRACT.md#draft-locus-invariant)). `inspect_unit` uses `INFORM_POPUP`, not map-selection | med | Add `InteractionKind` values by extending enum + registry row + client apply + title hook |
| 10 | Session state | `GameData.session_state_key` in [`game_data.toml`](games/hexdemo/resources/game_data.toml) | Pack module reads/writes ([`state/session_state.py`](games/hexdemo/state/session_state.py)); writes use [`BucketPatch`](src/hexengine/hooks/bucket.py) / [`ApplyBucketPatch`](src/hexengine/state/actions.py). Combat graph in [`combat/transitions.py`](games/hexdemo/combat/transitions.py). `last_combat`, `retreat_obligations`, `advance`, `disrupt_instead_offered` are **pack conventions**; legacy `combat_gate` is not written (cleared on phase advance) | [`GameState.session_state`](src/hexengine/state/game_state.py), `session_state_key`, [`engine_read_session_state`](src/hexengine/state/engine_session_state.py) / [`engine_write_session_state`](src/hexengine/state/engine_session_state.py); `engine_state` for `hexengine_*` keys (e.g. movement arc). Wire/snapshot: `session_state` + `session_state_key` (no combined `extension` map). `Attack.apply` is board+rng only; session handoff via `CombatOutcome` | med | — |
| 11 | Combat transitions & INFORM | — | [`combat/rules.py` BINDING](games/hexdemo/combat/rules.py) + [`combat/outcome.py`](games/hexdemo/combat/outcome.py); step loss in [`combat/step_loss.py`](games/hexdemo/combat/step_loss.py); disrupt in [`combat/disrupt.py`](games/hexdemo/combat/disrupt.py); gate `ui_mode` strings / phase clear in [`combat/transitions.py`](games/hexdemo/combat/transitions.py); cleanup mutations in [`combat/actions.py`](games/hexdemo/combat/actions.py); INFORM in [`hooks/ui.py`](games/hexdemo/hooks/ui.py) + [`presentation/`](games/hexdemo/presentation/) | [`authority_attack`](src/hexengine/server/arcs/authority_attack.py) (discovers Attack commit segment from arc graph, `submit_event`, `restore_routine_cursor` on overlay done), [`authority_combat_cleanup`](src/hexengine/server/arcs/authority_combat_cleanup.py) (`ArcSpec.advance_move_detector` only); engine `Attack` / `ApplyCombatEffects` apply board+rng and generic `unit_ops` only; `combat_event` fan-out from `UIHook.COMBAT_EVENT_SUMMARY`. **No hexdemo combat key literals in authority.** | none | — |
