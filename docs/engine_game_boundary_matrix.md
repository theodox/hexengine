...
| 4 | Turn schedule (game ↔ engine handshake) | — | [`GameDefinition.get_next_phase`](src/hexengine/gamedef/protocol.py) implemented in [`src/hexengine/gamedef/builtin.py`](src/hexengine/gamedef/builtin.py); [`advance_turn_action_for_state`](src/hexengine/gamedef/builtin.py) for tests/tooling | Server: [`_get_next_phase`](src/hexengine/server/game_server.py) → `_game_definition.get_next_phase`; [`NextPhase`](src/hexengine/server/game_server.py) only when title policy says so (`MovementHook.AUTO_ADVANCE_PHASE_AFTER_MOVE_SPEND`, `AttackHook.AUTO_ADVANCE_PHASE_AFTER_ATTACK`) or player clicks dock `end_phase`. Engine catalog default after move spend: advance when `phase_actions_remaining <= 0` | med | — |
| 5 | Action economy (handshake) | — | Title owns auto-advance policy via movement/attack hooks; hexdemo blocks advance during combat gates | [`GameServer._handle_action_request`](src/hexengine/server/game_server.py): after `execute(MoveUnit)`, `execute(SpendAction(1))`; then [`_maybe_auto_advance_phase`](src/hexengine/server/game_server.py) | med | Configurable spend amounts per action type |
