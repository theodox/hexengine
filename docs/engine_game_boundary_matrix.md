...
| 11 | Combat transitions & INFORM | — | [`combat/rules.py` BINDING](games/hexdemo/combat/rules.py) + [`combat/outcome.py`](games/hexdemo/combat/outcome.py); gate `ui_mode` strings / phase clear in [`combat/transitions.py`](games/hexdemo/combat/transitions.py); cleanup mutations in [`combat/actions.py`](games/hexdemo/combat/actions.py); INFORM in [`hooks/ui.py`](games/hexdemo/hooks/ui.py) + [`presentation/`](games/hexdemo/presentation/) | [`authority_attack`](src/hexengine/server/arcs/authority_attack.py) (discovers Attack commit segment from arc graph, `submit_event`, `restore_routine_cursor` on overlay done), [`authority_combat_cleanup`](src/hexengine/server/arcs/authority_combat_cleanup.py) (`ArcSpec.advance_move_detector` only); engine `Attack` / `ApplyCombatEffects` are board+rng only; `combat_event` fan-out from `UIHook.COMBAT_EVENT_SUMMARY`. **No hexdemo combat key literals in authority.** | none | — |
