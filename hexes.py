...
class HexCanvas:
    def __init__(self, canvas_id: str, hex_size):
        ...
        self.hex_layout = HexLayout(
            hex_size, self.canvas.width / 2, self.canvas.height / 2
        )
    ...
    def get_click_coords(self, event) -> tuple[float, float]:
        ...
        sx = self.canvas.width / rect.width
        sy = self.canvas.height / rect.height
        return (x *sx, y * sy)
    ...
    for x in range(-19, 20):
        for y in range(-19, 20):
            ...
            if hex in hex_canvas:
                ...
                hex_canvas.draw_text(
                    hex, f"{hex.i},{hex.j},{hex.k}", font="10px Arial", color="blue"
                )
