...
class SVGCanvas:
    def __init__(self, svg_element: js.SVGElement):
        ...
        self._click_handler = Handler(self._svg, "click")
        self._dblclick_handler = Handler(self._svg, "dblclick")
        self._drag_handler = Handler(self._svg, "mousemove")
        self._mouse_down_handler = Handler(self._svg, "mousedown")
        self._mouse_up_handler = Handler(self._svg, "mouseup")
...
class MapCanvas:
    ...
    def __init__(self, canvas_element: str):  
        ...
        self._drag_handler = Handler(self._canvas, "mousemove")
        self._mouse_down_handler = Handler(self._canvas, "mousedown")
        self._mouse_up_handler = Handler(self._canvas, "mouseup")
...
class Map:
    ...
    def __init__(self, 
        ...
        if self._hex_size is not None:
            ...
            self._hex_size = 24
        self._hex_layout = HexLayout(
            self._hex_size, canvas_element.width / 2, canvas_element.height / 2
        )
       
        self._hex_width = self._hex_size * 2
        self._hex_height = (3**0.5) * self._hex_size

        self._svg.on_click < self._canvas.on_click
        self._svg.on_dblclick < self._canvas.on_dblclick
