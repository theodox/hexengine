...
**Titles:** import by area — `hexengine.hooks.modification`, `.interaction`, `.bucket`, `.unit`, `.ui`,
`.title`, `.wiring`, `.core`.

**Wiring workflow**

1. Import hook **types** and the area **slot enum** from the matching module:
   `ModificationHook` (modification), `InteractionHook` (interaction),
   `UIHook` (values match `*Hooks` field names).
2. Import `bind_title_hook` from `hexengine.hooks.wiring`.
3. Decorate each implementation with `@bind_title_hook(ModificationHook.SOME_FIELD)` (etc.)
