...
class GameHistoryMixin:
    ...
    def enqueue(self, action: Action):
        ...
        logging.getLogger("History").debug(f"Added move: {action}, pointer at {self._history_pointer}")
    def has_moves(self):
    ...
    def undo(self):
        ...
        if self._history_pointer > 0:
            ...
            logging.getLogger("History").debug(f"Undid move: {move}, pointer at {self._history_pointer}")
            return move    
    ...
    def redo(self):
        ...
        if self._history_pointer < len(self._moves):
            ...
            self._history_pointer += 1
            move.do(self.board)
            logging.getLogger("History").debug(f"Redid move: {move}, pointer at {self._history_pointer}")
            return move
