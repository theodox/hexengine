...
class WebSocketGameServer:
    ...
    async def _handle_connection(self, websocket: WebSocketServerProtocol) -> None:
        ...
        self.logger.info(f"New connection from {websocket.remote_address}, assigned ID: {player_id}")
...
async def main():
    ...
    server = WebSocketGameServer(host="0.0.0.0", port=8765, initial_state=action_mgr.current_state)
    await server.start()
