...
def arc_event_action_types(arc: Arc) -> frozenset[str]:
    """All ``Event.action_type`` values declared on segment transitions in ``arc``."""

    types: set[str] = set()
    for seg in arc.segments:
        for transition in seg.transitions:
            trigger = transition.trigger
            if isinstance(trigger, Event) and str(trigger.action_type).strip():
                types.add(str(trigger.action_type))
